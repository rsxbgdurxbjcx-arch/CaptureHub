...
# tzdata:让容器内程序(TZ=Asia/Shanghai)正确显示本地时间
RUN apk add --no-cache ca-certificates curl tini ffmpeg openssl zlib libstdc++ su-exec tzdata font-noto-cjk \
    && mkdir -p /data /recordings /logs /config/rclone /data/scripts /home/node/.config/rclone

# 内置 rclone(固定 v1.75.1;官方静态二进制,支持 amd64/arm64,Alpine 可直接运行)
# 下载文件用官方 SHA256SUMS 校验,避免传输损坏或上游异常
...
RUN set -eux; \
    ...
    cd /tmp; \
    curl -fsSL --retry 3 -O "https://downloads.rclone.org/v${RCLONE_VERSION}/rclone-v${RCLONE_VERSION}-linux-${RCLONE_ARCH}.zip"; \
    curl -fsSL --retry 3 -o SHA256SUMS "https://downloads.rclone.org/v${RCLONE_VERSION}/SHA256SUMS"; \
    grep -E "  rclone-v${RCLONE_VERSION}-linux-${RCLONE_ARCH}\\.zip$" SHA256SUMS | sha256sum -c -; \
    unzip -q "rclone-v${RCLONE_VERSION}-linux-${RCLONE_ARCH}.zip"; \
    install -m 0755 "rclone-v${RCLONE_VERSION}-linux-${RCLONE_ARCH}/rclone" /usr/bin/rclone; \
    rm -rf /tmp/rclone-v${RCLONE_VERSION}-linux-${RCLONE_ARCH} /tmp/rclone-v${RCLONE_VERSION}-linux-${RCLONE_ARCH}.zip /tmp/SHA256SUMS; \
