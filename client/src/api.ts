...
export const api = {
  ...
  // -- rclone 网盘配置(账号密码自动生成 rclone.conf) --
  getRcloneStatus: (force = false) =>
    request<RcloneStatus>(`/api/postprocess/rclone/status${force ? '?force=1' : ''}`),
  connectRclone: (body: { provider: string; account: string; password: string }) =>
    request<{ provider: string; providerLabel: string; remote: string }>(
      '/api/postprocess/rclone/connect',
