...
/** 正在手动上传的文件 id 集合(防止连点重复上传) */
const busyUploading = ref<Set<string>>(new Set());
...
async function onRefresh() {
  ...
  try {
    // 下拉刷新 = 精确同步:先触发后端磁盘扫描(异步),再加载最新列表。
    // 普通轮询不再默认全盘扫描(节流),避免文件多时拖垮 Web UI
    await api.syncFiles();
  } catch { /* 同步失败时仍返回当前列表 */ }
  await load(true);
}
...
async function upload(f: RecordingFile) {
  ...
  // 防重复点击:同一文件上传中忽略再次点击(后端另有队列/状态双保险)
  if (busyUploading.value.has(f.id)) return;
  const nextBusy = new Set(busyUploading.value);
  nextBusy.add(f.id);
  busyUploading.value = nextBusy;
  try {
    toast.info('正在上传...');
    ...
    const next = new Set(busyUploading.value);
    next.delete(f.id);
    busyUploading.value = next;
  }
}

function canPlay(f: RecordingFile) {
  if (f.status === 'recording') return false;
  // 后端已不在响应中回传 absolutePath,改由 relativePath 判断
      ...
      >
        ...
        <div class="actions">
          ...
          <button
            ...
            :disabled="f.status === 'recording' || f.status === 'processing' || busyUploading.has(f.id)"
            @click="upload(f)"
          >
            {{ busyUploading.has(f.id) ? '上传中...' : '上传' }}
          </button>
