...
const form = ref({
  ...
  /** 录制弹幕开关(默认关闭;支持虎牙/斗鱼/B站, 弹幕将烧录进视频) */
  recordDanmaku: false,
});
...
function autoDetectPlatform(url: string): Platform {
  ...
  if (/douyu/i.test(url)) return 'douyu';
  if (/huya/i.test(url)) return 'huya';
...
function openEdit(s: Streamer) {
  ...
  form.value = {
    ...
    recordDanmaku: s.recordDanmaku ?? false,
  };
  modalOpen.value = true;
}
...
async function save() {
  ...
  try {
    const body = {
      ...
      recordDanmaku: form.value.recordDanmaku,
    };
    if (editing.value) {
...
const isDouyu = computed(() => form.value.platform === 'douyu');
const isHuya = computed(() => form.value.platform === 'huya');
const isBigo = computed(() => form.value.platform === 'bigo');
...
const urlLabel = computed(() => {
  ...
  if (isDouyu.value) return '斗鱼直播间 / 主播主页链接 *';
  if (isHuya.value) return '虎牙直播间 / 主播主页链接 *';
  if (isBigo.value) return 'Bigo Live 主播主页 / 直播链接 *';
...
const urlPlaceholder = computed(() => {
  ...
  if (isDouyu.value) return '如 www.douyu.com/xxx 或 www.douyu.com/123456';
  if (isHuya.value) return '如 www.huya.com/123456 或 www.huya.com/别名';
  if (isBigo.value) return '如 www.bigo.tv/123456';
  return '小红书链接...';
});
...
<template>
  <div class="streamers-view">
    ...
    <div v-else-if="!hasStreamers" class="md-card empty">
      暂无主播,请添加小红书、抖音、哔哩哔哩、快手、SOOP、PandaLive、Stripchat、斗鱼、虎牙、Bigo Live 主页/直播间链接
    </div>
        ...
        <div v-if="modalOpen" class="modal-mask" @click.self="modalOpen = false">
          <div class="modal streamer-modal">
            ...
            <div class="switch">
              <div>
                <div>录制弹幕</div>
                <div class="muted">弹幕烧录进视频并保留 XML(虎牙/斗鱼/B站)</div>
              </div>
              <label class="toggle">
                <input v-model="form.recordDanmaku" type="checkbox" /><span class="slider" />
              </label>
            </div>
            <div class="switch">
...
.platform-badge.douyu { background: oklch(0.62 0.19 40 / 85%); }
.platform-badge.huya { background: oklch(0.58 0.17 240 / 85%); }
.platform-badge.bigo { background: oklch(0.6 0.18 285 / 85%); }
