...
streamersRouter.delete('/:id', (req, res) => {
  const cur = streamerRepo.get(req.params.id);
  if (!cur) {
    res.status(404).json({ ok: false, error: '主播不存在' } satisfies ApiResponse);
    return;
  ...
  if (recorderService.isRecording(cur.id)) {
    // 后台异步停止录制,立即返回,避免删除主播按钮卡顿
    void recorderService.stop(cur.id, 'manual_stop').catch((e) => {
...
/** 批量删除主播 */
streamersRouter.post('/batch/delete', (req, res) => {
