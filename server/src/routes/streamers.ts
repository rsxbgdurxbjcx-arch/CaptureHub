...
import { buildPlatformHeaderArgs } from '../services/platform-headers.js';
...
function detectPlatform(url: string): Platform {
  ...
  if (/bigo\.tv/i.test(url)) return 'bigo';
  return 'xhs';
}
...
const urlLike = z
  ...
  .refine(
    (v) =>
      /^(https?:\/\/)?([\w-]+\.)?(xiaohongshu\.com|xhslink\.com|douyin\.com|v\.douyin\.com|bilibili\.com|live\.bilibili\.com|space\.bilibili\.com|kuaishou\.com|live\.kuaishou\.com|sooplive\.com|sooplive\.co\.kr|afreecatv\.com|pandalive\.co\.kr|stripchat\.com|douyu\.com|huya\.com|bigo\.tv)\//i.test(
        v.trim(),
      ),
    '请输入有效的小红书、抖音、哔哩哔哩、快手、SOOP、Pandalive、Stripchat、斗鱼、虎牙或 Bigo Live 链接',
  );

const upsertSchema = z.object({
  name: z.string().optional(),
  profileUrl: urlLike,
  platform: z.enum(['xhs', 'douyin', 'bilibili', 'kuaishou', 'soop', 'pandalive', 'stripchat', 'douyu', 'huya', 'bigo']).optional(),
  ...
  /** 录制弹幕开关 (仅虎牙/斗鱼支持) */
  recordDanmaku: z.boolean().optional(),
...
/** 进行中的抓帧任务:并发/密集刷新时复用同一任务,避免重复拉起 ffmpeg 或返回 503 */
const snapshotPending = new Map<string, Promise<Buffer>>();
...
function captureFrame(
  ...
  return new Promise((resolve) => {
    ...
    const args = [
      ...
      '-loglevel', 'error',
      '-rw_timeout', '4000000',
      // 平台 CDN 特征头(如 B 站 pt=html5 流需 Referer/UA, 否则部分节点 403)
      ...extraInputArgs,
      '-i', streamUrl,
      '-frames:v', '1',
      '-vf', 'scale=480:-2',
...
streamersRouter.get('/:id/snapshot', async (req, res) => {
  ...
  // 并发/密集刷新:复用同一进行中的抓帧任务(等待其完成),避免重复拉起 ffmpeg
  let task = snapshotPending.get(id);
  if (!task) {
    const settings = loadSettings();
    const streamer = streamerRepo.get(id);
    const headerArgs = buildPlatformHeaderArgs(streamer?.platform, settings);
    task = captureFrame(settings.ffmpegPath, streamUrl, headerArgs).finally(() => {
      snapshotPending.delete(id);
    });
    snapshotPending.set(id, task);
  }
  try {
    ...
    if (buf.length > 0) {
      ...
      let host = '';
      try { host = new URL(streamUrl).host; } catch { /* ignore */ }
      console.warn(`[snapshot] 快照捕获失败 streamer=${id} host=${host}`);
...
streamersRouter.post('/', async (req, res) => {
  try {
    ...
    if (platform === 'douyin') {
      ...
      info = await parser.resolveFromProfileUrl(profileUrl);
    } else if (platform === 'douyu') {
      const parser = new DouyuParser();
      if (!parser.matchURL(profileUrl)) {
        res.status(400).json({
          ok: false,
          error: '请输入斗鱼直播间/主播主页链接 (douyu.com)',
        } satisfies ApiResponse);
        return;
      }
      info = await parser.resolveFromProfileUrl(profileUrl);
      ...
      if (!parser.matchURL(profileUrl)) {
        res.status(400).json({
          ok: false,
          error: '请输入虎牙直播间/主播主页链接 (huya.com)',
        } satisfies ApiResponse);
        return;
      }
      info = await parser.resolveFromProfileUrl(profileUrl);
      ...
      if (!parser.matchURL(profileUrl)) {
        res.status(400).json({
          ok: false,
          error: '请输入 Bigo Live 主播主页/直播间链接 (bigo.tv)',
        } satisfies ApiResponse);
        return;
      }
      info = await parser.resolveFromProfileUrl(profileUrl);
    ...
    const streamer: Streamer = {
      ...
      recordDanmaku: parsed.recordDanmaku ?? false,
      lastError: null,
      lastCheckedAt: now,
...
streamersRouter.put('/:id', async (req, res) => {
  try {
    ...
    let patch: Partial<Streamer> = {
      ...
      recordDanmaku: parsed.recordDanmaku ?? cur.recordDanmaku,
...
async function removeStreamerWithFiles(
  ...
  for (const f of files) {
    if (f.absolutePath) {
      ...
      // 同步删除同名弹幕 XML (弹幕文件与视频段同名同目录)
      const xmlPath = f.absolutePath.replace(/\.[^./\\]+$/, '.xml');
      if (xmlPath !== f.absolutePath) {
        markPathPendingDelete(xmlPath);
        try {
  ...
  for (const d of dirsToCleanup) {
    try {
      ...
      // 弹幕 XML 为该主播的录制产物(move 上传后视频已删、记录已移除),
      // 删除主播时一并清理, 避免孤儿弹幕文件阻塞目录清理
      for (const n of fs.readdirSync(d)) {
        if (n.endsWith('.xml')) {
          try {
            fs.unlinkSync(path.join(d, n));
          } catch {
            // ignore
          }
        }
