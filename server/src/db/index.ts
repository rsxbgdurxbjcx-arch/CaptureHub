...
/**
 * "待删除文件"墓碑集合(内存):
 * move 上传/删除主播等场景删除文件记录前,先将绝对路径登记到此集合,
 * syncFilesFromDisk 扫描时跳过这些路径,避免文件删除稍慢(如 unlink 重试期间)
 * 就被磁盘同步重新登记成"幽灵/复活"卡片。条目在对应文件已不存在时自动清除。
 */
const pendingDeletePaths = new Set<string>();

/** 登记某路径为"已发起删除"(文件允许仍短暂存在) */
export function markPathPendingDelete(p: string) {
  if (!p) return;
  pendingDeletePaths.add(p);
}

/** 判断某路径是否处于"已发起删除"状态(磁盘同步应跳过) */
export function isPathPendingDelete(p: string): boolean {
  return pendingDeletePaths.has(p);
}

/** 清除登记(文件确认已不存在时调用;同步流程内自动清理) */
export function unmarkPathPendingDelete(p: string) {
  pendingDeletePaths.delete(p);
}
...
function migrate() {
  ...
  // Migration: add record_danmaku column for existing databases
  // (录制弹幕开关;仅虎牙/斗鱼支持,默认 0 关闭)
  try {
    const cols = getDb().prepare('PRAGMA table_info(streamers)').all() as Array<{ name: string }>;
    if (!cols.some((c) => c.name === 'record_danmaku')) {
      getDb().exec('ALTER TABLE streamers ADD COLUMN record_danmaku INTEGER NOT NULL DEFAULT 0');
    }
  } catch {
    // ignore migration errors
  }
...
function rowToStreamer(row: any): Streamer {
  return {
    ...
    recordDanmaku: !!row.record_danmaku,
    ...
    getDb()
      .prepare(
        `INSERT INTO streamers (
          ...
          status, enabled, downloader, record_quality, record_danmaku, last_error, last_checked_at, last_live_at,
          created_at, updated_at
        ) VALUES (
          @id, @name, @profileUrl, @platform, @roomId, @userId, @redId, @avatar, @avatarUpdatedAt, @title,
          @status, @enabled, @downloader, @recordQuality, @recordDanmaku, @lastError, @lastCheckedAt, @lastLiveAt,
          @createdAt, @updatedAt
        )`,
      )
      .run({
        ...
        recordDanmaku: s.recordDanmaku ? 1 : 0,
      });
  },
    ...
    getDb()
      .prepare(
        `UPDATE streamers SET
          ...
          enabled=@enabled, downloader=@downloader, record_quality=@recordQuality, record_danmaku=@recordDanmaku,
          last_error=@lastError, last_checked_at=@lastCheckedAt, last_live_at=@lastLiveAt,
      ...
      .run({
        ...
        recordDanmaku: next.recordDanmaku ? 1 : 0,
      });
    return next;
  },
  remove(id: string) {
...
export function syncFilesFromDisk(recordingsDir: string) {
  ...
  const walk = (dir: string, base = '') => {
    ...
    for (const name of entries) {
      ...
      // 跳过隐藏文件/目录(如 .tg_compat_*.mp4 临时转码文件、stripchat 分片临时目录)
      if (name.startsWith('.')) continue;
      if (!/\.(mp4|mkv|ts|flv|m4a)$/i.test(name)) continue;
      // 已登记"待删除"的文件:不再自动登记记录,防止 move 删除期间复活幽灵卡片
      if (isPathPendingDelete(abs)) continue;
  ...
  // 清理墓碑:文件已不存在(删除已生效)的登记条目及时移除,避免集合无限增长
  for (const p of pendingDeletePaths) {
    if (!fs.existsSync(p)) pendingDeletePaths.delete(p);
  }
}
