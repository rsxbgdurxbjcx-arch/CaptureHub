...
import { BigoParser } from '../bigo/parser.js';
 ...
 * SOOP(soop)、Pandalive(pandalive)、Stripchat(stripchat)、
 * 斗鱼(douyu)、虎牙(huya)、Bigo Live(bigo) 十平台
 */
export class MonitorService {
  ...
  private getPlatformCookie(platform: Platform): string {
    ...
    switch (platform) {
      ...
      case 'stripchat':
        // Stripchat 不允许多设备同时登录,Cookie 无使用场景
        return '';
      case 'douyu':
      case 'huya':
      case 'bigo':
        // 斗鱼/虎牙/Bigo 拉流不使用 Cookie (与 biliup 一致)
        return '';
      case 'xhs':
      default:
  ...
  private async checkStreamer(streamer: Streamer) {
    ...
    if (platform === 'douyin') {
      ...
      await this.checkDouyuStreamer(streamer);
    } else if (platform === 'huya') {
      await this.checkHuyaStreamer(streamer);
    } else if (platform === 'bigo') {
      await this.checkBigoStreamer(streamer);
    } else {
  ...
  /**
   * 斗鱼主播监控逻辑
   * 斗鱼用真实房间号(roomId)作为稳定标识,通过 betard 接口检测直播状态
   */
  private async checkDouyuStreamer(streamer: Streamer) {
    ...
    if (!roomId) {
      try {
        ...
        console.warn(
          `[monitor] douyu resolve failed for ${streamer.name}:`,
          e instanceof Error ? e.message : e,
        );
      }
    }
    ...
    try {
      ...
      if (info.title) title = info.title;
      if (info.owner) owner = info.owner;
      // getRoomInfo 在播时已附带取流(供快照/开录复用)
      streamUrl = info.flvUrl || null;
      if (living) {
        console.log(`[monitor] douyu detect: ${streamer.name} is LIVE roomId=${roomId}`);
      }
    } catch (e) {
      console.warn(
        `[monitor] douyu check failed for ${streamer.name}:`,
        e instanceof Error ? e.message : e,
      );
      streamerRepo.update(streamer.id, {
        status: 'parse_error',
    ...
    // 在播但流地址缺失(如取流瞬时失败)时补取一次
    if (!streamUrl) {
      try {
        const streams = await parser.getStreams(roomId, ['flv']);
        streamUrl = streams[0]?.streams[0]?.url || null;
        if (streamUrl) recorderService.setOnlineStreamUrl(streamer.id, streamUrl);
      } catch (e) {
        console.warn(
          `[monitor] douyu getStreams failed for ${streamer.name}:`,
          e instanceof Error ? e.message : e,
        );
      }
    }
  ...
  /**
   * 虎牙主播监控逻辑
   * 虎牙用真实房间号(roomId)作为稳定标识,通过房间页 + WUP 取流检测
   */
  private async checkHuyaStreamer(streamer: Streamer) {
    const parser = new HuyaParser();
    ...
    if (!roomId) {
      try {
        ...
        console.warn(
          `[monitor] huya resolve failed for ${streamer.name}:`,
          e instanceof Error ? e.message : e,
        );
      }
    }
    ...
    try {
      ...
      streamUrl = info.flvUrl || null;
      if (living) {
        console.log(`[monitor] huya detect: ${streamer.name} is LIVE roomId=${roomId}`);
      }
    } catch (e) {
      console.warn(
        `[monitor] huya check failed for ${streamer.name}:`,
        e instanceof Error ? e.message : e,
      );
      streamerRepo.update(streamer.id, {
        status: 'parse_error',
    ...
    // 按主播画质设置取流: 非 OD(原画)时按码率上限重新取流,
    // OD 直接复用 getRoomInfo 已获取的地址(避免重复请求)
    if (living && streamer.recordQuality && streamer.recordQuality !== 'OD') {
      try {
        const streams = await parser.getStreams(roomId, ['flv'], streamer.recordQuality);
        const q = streams[0]?.streams[0]?.url || null;
        if (q) streamUrl = q;
      } catch (e) {
        console.warn(
          `[monitor] huya getStreams(${streamer.recordQuality}) failed for ${streamer.name}:`,
          e instanceof Error ? e.message : e,
        );
      }
    }
    ...
    if (!streamUrl) {
      try {
        ...
        console.warn(
          `[monitor] huya getStreams failed for ${streamer.name}:`,
          e instanceof Error ? e.message : e,
        );
      }
    }
  ...
  /**
   * Bigo Live 主播监控逻辑
   * Bigo 用 siteId 作为稳定标识,通过官方 studio 接口检测直播状态
   */
  private async checkBigoStreamer(streamer: Streamer) {
    const parser = new BigoParser();

    let siteId: string | null = streamer.userId || streamer.roomId;

    // 如果没有 siteId,尝试从 profileUrl 重新解析
    if (!siteId) {
      try {
        const info = await parser.resolveFromProfileUrl(streamer.profileUrl);
        siteId = info.userId;
        ...
        console.warn(
          `[monitor] bigo resolve failed for ${streamer.name}:`,
          e instanceof Error ? e.message : e,
        );
      }
    }
    ...
    if (!siteId) {
      streamerRepo.update(streamer.id, {
        status: 'parse_error',
        lastError: '无法获取 Bigo 用户 ID，请确认链接正确',
        lastCheckedAt: nowIso(),
      });
      return;
    }
    ...
    try {
      const info = await parser.getRoomInfo(siteId);
      living = info.living;
      if (info.title) title = info.title;
      if (info.owner) owner = info.owner;
      streamUrl = info.m3u8Url || null;
      if (living) {
        console.log(`[monitor] bigo detect: ${streamer.name} is LIVE siteId=${siteId}`);
      }
    } catch (e) {
      console.warn(
        `[monitor] bigo check failed for ${streamer.name}:`,
        e instanceof Error ? e.message : e,
      );
      streamerRepo.update(streamer.id, {
        status: 'parse_error',
    ...
    if (!streamUrl) {
      try {
        const streams = await parser.getStreams(siteId, ['hls']);
        streamUrl = streams[0]?.streams[0]?.url || null;
        if (streamUrl) recorderService.setOnlineStreamUrl(streamer.id, streamUrl);
      } catch (e) {
        console.warn(
          `[monitor] bigo getStreams failed for ${streamer.name}:`,
          e instanceof Error ? e.message : e,
        );
      }
    }
