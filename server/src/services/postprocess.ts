...
import { renderDanmakuVideo } from '../danmaku/burn.js';
import { danmakuService } from '../danmaku/service.js';
import { grammyService, generateTags } from './grammy.js';
import { isValidRemoteName, isValidRemotePath, sanitizeRcloneText, markRcloneConnected } from './rclone.js';
...
/**
 * "结果未知"错误判定:超时类错误不保证请求未被服务端处理,
 * 自动重试可能把同一视频再次投递到 Telegram → 重复上传。
 * 此类错误改为"由用户确认后手动处理",系统不做自动重试。
 */
function isUnknownOutcomeError(msg: string): boolean {
...
export class PostProcessService {
  ...
  /** 正在执行后处理的文件 id 集合(防止同一文件被并发/排队重复上传) */
  ...
  enqueue(fileId: string, trigger: PostProcessTrigger, attempt = 0) {
    // 防重复入队:同一文件已在执行中或已排队时丢弃新任务。
    // 修复重复上传根因之一:同一段文件可能被多次触发(下播+手动停止+切片、
    // 手动上传+自动任务、失败重试+手动点击),旧逻辑排队两个任务分别执行,
    // 第一个成功后第二个又上传一遍。
    if (this.inFlight.has(fileId) || this.queue.some((q) => q.fileId === fileId)) {
      console.warn(`[postprocess] 忽略重复入队: ${fileId} (已有任务在队列/执行中)`);
      return;
    }
  ...
  async runNow(fileId: string, trigger: PostProcessTrigger = 'manual') {
    // 同一文件正在处理中:直接拒绝,避免"手动上传 + 自动任务"并发重复上传
    if (this.inFlight.has(fileId)) {
      throw new Error('该文件正在后处理中,请稍候');
    }
  ...
  private async pump() {
    ...
    while (this.queue.length > 0 && this.activeCount < this.getMaxConcurrent()) {
      ...
      // 同一文件已在执行中(理论上队列已去重,此处为二次防护):跳过该任务
      if (this.inFlight.has(item.fileId)) {
        console.warn(`[postprocess] 跳过重复任务: ${item.fileId} (执行中)`);
        continue;
      }
      this.activeCount++;
      this.inFlight.add(item.fileId);
  ...
  private async execute(fileId: string, trigger: PostProcessTrigger, attempt = 0) {
    ...
    // 日志落库节流:部分 ffmpeg 输出会按字形/帧粒度刷屏(如 libass 对弹幕中
    // emoji 的缺字形提示, 单条视频可达数万行)。若每行都全量 UPDATE log 字段,
    // 会产生 O(n²) 写放大拖垮磁盘 IO 与事件循环(实测曾达 ~100MB/s 持续写入,
    // 导致 API 全部超时、UI 卡死)。合并为最多 1s 写一次并限制日志上限。
    const LOG_FLUSH_INTERVAL_MS = 1000;
    const LOG_MAX_CHARS = 256 * 1024;
    let log = '';
    let logDirty = false;
    ...
    const append = (line: string) => {
      log += line + '\n';
      if (log.length > LOG_MAX_CHARS) {
        log = '...(较早日志已截断)\n' + log.slice(-LOG_MAX_CHARS);
      }
      logDirty = true;
      if (!logTimer) {
        logTimer = setTimeout(flushLog, LOG_FLUSH_INTERVAL_MS);
        logTimer.unref();
      }
    };
    ...
    try {
      ...
      // 弹幕烧录:开启录制弹幕且存在同名弹幕文件时,把弹幕渲染进画面(重编码为 mp4)
      let burned = false;
      const danmakuXml = this.resolveDanmakuXml(file);
      if (danmakuXml) {
        const srcBytes =
          file.size ||
        ...
        if (freeBytes < srcBytes * 1.5 + 256 * 1024 * 1024) {
          append(`[danmaku] 磁盘空间不足,跳过弹幕烧录,按普通流程处理`);
        } else {
          const burnedFile = await this.burnDanmakuToMp4(
            file,
            danmakuXml,
            settings.ffmpegPath,
            append,
      ...
      if (!burned && settings.autoTranscode && !file.filename.toLowerCase().endsWith('.mp4')) {
      ...
      // move 模式:上传完成 → 先标记 uploaded(语义上"已上传成功"),
      // 再删除本地文件与记录。即使后续清理失败,记录也绝不会被再次上传
      // (修复重复上传根因之一:此前"上传成功→清理异常"会被标记 error,
      //  用户/自动重试会再次上传同一视频)。
      const isMoveMode =
      ...
      let deleteFailed = '';
      if (isMoveMode) {
        // 1) 先标记 uploaded:任何后续清理异常都不会造成重复上传
        const stillExistsAtMark =
          !!workFile.absolutePath && fs.existsSync(workFile.absolutePath);
        fileRepo.update(workFile.id, {
          status: 'uploaded',
          uploadedAt: nowIso(),
          absolutePath: stillExistsAtMark ? workFile.absolutePath : '',
          size: stillExistsAtMark ? fs.statSync(workFile.absolutePath).size : 0,
          error: null,
        });

        // 2) 删除本地物理文件(带重试,确保真正删除;rclone/grammy move 可能已删)
        ...
        // 3) 删除文件记录(move:卡片随文件一起消失;失败仅告警,
        //    记录保留为 uploaded,不会再次上传)
        ...
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          append(
            `[cleanup] 删除文件记录失败(记录已保留为"已上传"状态,不会重复上传): ${msg}`,
          );
          console.warn(`[postprocess] 删除文件记录失败(已保留为 uploaded): ${msg}`);
        }
      ...
      // 上传已成功(标记 uploaded)后发生的清理类异常:不再覆盖为 error,
      // 否则用户/自动重试会再次上传同一视频(重复上传根因之一)
      const latestRec = fileRepo.get(file.id);
      if (latestRec && latestRec.status === 'uploaded') {
        append('[warn] 文件已上传成功,仅记录清理异常,不标记失败、不重试');
        jobRepo.update(jobId, {
          status: 'success',
          log,
          finishedAt: nowIso(),
      ...
      // 结果未知错误(超时等):不自动重试 —— 请求可能已被服务端处理,
      // 自动重试会把同一视频二次投递到 Telegram;改为标记 error 并提示用户
      // 先确认群组是否已收到,确认未收到后再手动重试。
      const unknownOutcome = isUnknownOutcomeError(msg);

      // 本地文件仍完好且非"结果未知"错误时自动延迟重试(网络/限流等瞬时错误),
      // 最多 POSTPROCESS_MAX_RETRY 次;全部失败后才标记永久 error。
      const fileStillExists = !!file.absolutePath && fs.existsSync(file.absolutePath);
      const canRetry =
        attempt < POSTPROCESS_MAX_RETRY && fileStillExists && !unknownOutcome;
      ...
      if (canRetry) {
        ...
        fileRepo.update(file.id, {
          ...
          error: unknownOutcome
            ? `${msg}\n【注意】超时不代表未收到:请先在 Telegram 确认视频是否已上传成功,` +
              `确认未收到后再手动重试(系统不会自动重试超时错误,避免重复上传)`
            : msg,
        });
        jobRepo.update(jobId, {
          status: 'failed',
  ...
  private async executeRcloneUpload(
    ...
    // remote 名与网盘根目录校验(防注入/路径穿越):不合法直接失败,不执行脚本
    if (!isValidRemoteName(settings.rcloneRemote) || !isValidRemotePath(settings.rcloneRemotePath)) {
      throw new Error('rclone 远程名或网盘根目录不合法,请到「后处理 → rclone」重新保存配置');
    }
    ...
    const { done } = runCommand('sh', [scriptPath], {
      ...
      onStdout: (raw) => {
        const line = sanitizeRcloneText(raw);
        append(`[out] ${line}`);
  ...
  /**
   * 判断文件是否需要弹幕烧录: 主播开启录制弹幕且存在同名非空 XML 时返回其路径。
   */
  private resolveDanmakuXml(file: RecordingFile): string | null {
    if (!file.streamerId) return null;
    const streamer = streamerRepo.get(file.streamerId);
    if (!streamer || !streamer.recordDanmaku) return null;
    if (!danmakuService.supports(streamer.platform)) return null;
    const xmlPath = file.absolutePath.replace(/\.[^./\\]+$/, '.xml');
    try {
      const st = fs.statSync(xmlPath);
      if (!st.isFile() || st.size < 64) return null;
      // 已烧录过(如上传失败重试再次进入)不再重复烧录, 避免弹幕重影
      if (fs.readFileSync(xmlPath, 'utf8').includes('danmaku-burned')) return null;
    } catch {
      return null;
    }
    return xmlPath;
  ...
  /**
   * 弹幕烧录: 渲染同名 XML 弹幕到画面并输出 mp4(替代源文件)。
   * 与 transcodeToMp4 同套路: 先更新记录指向目标路径再处理,
   * 失败回滚记录并返回 null, 由调用方走普通转码/上传流程。
   */
  private async burnDanmakuToMp4(
    file: RecordingFile,
    xmlPath: string,
    ffmpegPath: string,
    append: (s: string) => void,
    ...
    const rel = path
      .relative(loadSettings().recordingsDir, outPath)
      .replace(/\\/g, '/');
    // 临时输出用隐藏名(磁盘同步跳过), 成功后原子替换:
    // 兼容"源文件已是 mp4"的情况, 避免 ffmpeg 输入输出同路径
    const tmpOut = path.join(dir, `.${base}.danmaku.tmp`);

    // 与 transcodeToMp4 相同:先更新文件记录指向目标路径,防止磁盘同步产生幽灵记录
    fileRepo.update(file.id, {
      filename: outName,
      absolutePath: outPath,
      relativePath: rel,
    ...
    try {
      const platform = file.streamerId ? streamerRepo.get(file.streamerId)?.platform : undefined;
      const ok = await renderDanmakuVideo({
      ...
      if (path.resolve(file.absolutePath) !== path.resolve(outPath)) {
        try {
          fs.unlinkSync(file.absolutePath);
        } catch { /* ignore */ }
      }
      // 给 XML 打标记: 上传失败重试时跳过重复烧录(否则弹幕会重影)
      try {
        fs.appendFileSync(xmlPath, `<!-- danmaku-burned ${Date.now()} -->\n`);
      } catch { /* ignore */ }
      const size = fs.statSync(outPath).size;
      const updated = fileRepo.update(file.id, { size, status: 'processing' });
      return updated || file;
    } catch (e) {
      rollback();
      append(`[danmaku] 烧录异常,回退普通流程: ${e instanceof Error ? e.message : e}`);
      return null;
    }
  }
