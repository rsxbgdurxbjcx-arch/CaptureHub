...
import { buildPlatformHeaderArgs } from './platform-headers.js';
...
export class RecorderService {
  ...
  async start(opts: {
    ...
    // 弹幕录制: 仅虎牙/斗鱼, 且主播开启了录制弹幕开关 (XML 与视频段同名同目录)
    if (streamer.recordDanmaku) {
      const rec = this.active.get(streamer.id);
      if (rec) {
        danmakuService.startFor({
          streamerId: streamer.id,
          platform: streamer.platform,
          roomId,
          videoPath: rec.currentFilePath,
        });
      }
    }
    return this.active.get(streamer.id)!;
  ...
  private async spawnSegment(
    ...
    child.on('close', async (code) => {
      ...
      if (existed) {
        ...
        // 切片: 先开新段并滚动弹幕(收尾本段 XML), 再触发本段后处理
        if (!current.stopping && isRealSegment) {
          session.segmentIndex += 1;
          try {
            await this.spawnSegment(session, 'stream_end');
            const next = this.active.get(session.streamer.id);
            if (next) danmakuService.rollTo(session.streamer.id, next.currentFilePath);
        ...
        // 非切片(停止/流结束)或切片失败: 先收尾弹幕(flush 缓冲、补 </i>),
        // 再触发后处理: 后处理会读取同名 XML 烧录/上传, 必须读到完整文件
        danmakuService.stop(session.streamer.id);
        postProcessService.enqueue(fileId, trigger);
      } else {
  ...
  private finalize(rec: ActiveRecording, reason: PostProcessTrigger) {
    ...
    // 停止该主播的弹幕录制并收尾 XML (无会话时为 no-op)
    danmakuService.stop(rec.streamerId);
  ...
  private async runStripchatLoop(session: StripchatSession) {
    ...
    while (!abort.signal.aborted) {
      ...
      const segmentFileSizeBytes = parseFileSizeToBytes(settings.segmentFileSize);
      try {
        const playlist = await fetchPlaylist(currentPlaylistUrl);
        ...
        if (initUrl && initUrl !== cachedInitUrl) {
          try {
            mp4Header = await downloadSegment(initUrl);
        ...
        for (const segUrl of segments) {
          ...
          try {
            const bytes = await downloadSegment(segUrl);
    ...
    const trySpawn = (type: DownloaderType): ChildProcess | null => {
      try {
        if (type === 'ffmpeg') {
          ...
          // 各平台 CDN 特征头(Referer/Origin/UA 等), 与快照抓帧共用同一构造逻辑
          args.push(...buildPlatformHeaderArgs(platform, settings));
