...
/** 文件大小字符串(MB) → 字节数,空字符串返回 0 */
export function parseFileSizeToBytes(input: string | number): number {
  if (typeof input === 'number' && Number.isFinite(input)) {
    return Math.max(0, Math.floor(input * 1024 * 1024));
  }
  const s = String(input || '').trim();
  if (!s) return 0;
...
/**
 * 获取目录所在文件系统的剩余可用字节数。
 * statfs 不可用(如某些受限容器/非 POSIX 文件系统)时返回 Number.MAX_SAFE_INTEGER,
 * 表示"不限制",避免误报磁盘不足。
 */
export function getDiskFreeBytes(dir: string): number {
  try {
...
export function runCommand(
  ...
  const bind = (
    ...
    stream.on('data', (chunk: Buffer) => {
      buf += chunk.toString();
      // 同时识别 \r\n / \n / \r 三种行分隔符:
      // rclone 的 --stats-one-line 每 1 秒以回车符 \r 覆盖输出单行(不以 \n 结尾),
      // 若只按 \n 切分,实时进度行会全部滞留缓冲区、直到进程结束才触发一次,
      // 导致"处理中"卡片永远收不到真实上传进度/速度/进度条。
