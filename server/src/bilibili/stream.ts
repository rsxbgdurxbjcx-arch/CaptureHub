...
/** 画质受限警告去重 (按房间; 恢复高画质后自动清除) */
const qualityWarned = new Set<number>();
...
export async function getStream(opts: {
  ...
  // 画质受限提示(按房间去重): B 站未登录/账号受限时会降级画质(如只有 250 超清=720p);
  // 在设置中更新 B 站 Cookie(登录态)可解锁原画等更高档位
  if (liveInfo.current_qn < qn) {
    if (!qualityWarned.has(roomId)) {
      qualityWarned.add(roomId);
      console.warn(
        `[bilibili] 画质受限: 请求 qn=${qn}, 实际 qn=${liveInfo.current_qn}` +
          (opts.cookie
            ? '(B 站 Cookie 可能已失效, 请到设置中更新以解锁更高画质)'
            : '(未配置 B 站 Cookie)'),
      );
    }
  } else {
    qualityWarned.delete(roomId);
  }
