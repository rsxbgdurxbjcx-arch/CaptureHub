...
export async function getInfo(
  ...
  const cam = await fetchCamInfo(username);
  ...
  const groupShowType =
    status === 'groupShow' ? await fetchGroupShowType(username) : null;
...
export async function getStream(opts: {
  ...
  const { text, baseUrl } = await fetchMasterPlaylist(modelId);
...
export async function getInfoAndStream(
  username: string,
  opts: { quality?: string; mouflonKeys?: Record<string, string> } = {},
