...
function buildBrowserHeaders(
  opts: { referer?: string; page?: boolean } = {},
...
function buildCdnHeaders(): Record<string, string> {
  const headers: Record<string, string> = {
    'User-Agent': USER_AGENT,
    Accept: '*/*',
    Referer: REFERER,
...
export async function fetchCamInfo(
  ...
  try {
    const bResp = await doGet(
      ...
      { headers: buildBrowserHeaders({ referer: profileReferer(username) }) },
    );
    if (bResp.status === 404) {
    ...
    if (!bResp.ok) {
      ...
      if (streamName) {
        return await fetchCamByModelId(streamName, username);
      }
  ...
  try {
    const fromPage = await fetchCamInfoFromPage(username);
...
async function fetchCamInfoFromPage(
  ...
  for (const url of candidates) {
    const resp = await doGet(url, {
      headers: buildBrowserHeaders({ referer: url, page: true }),
    });
    if (!resp.ok) continue;
...
export async function fetchGroupShowType(
  ...
  while (true) {
    ...
    const headers = buildBrowserHeaders({ referer: REFERER });
 ...
 */
async function cdnGet(url: string): Promise<Response> {
 ...
 */
export async function fetchPlaylist(playlistUrl: string): Promise<string> {
  const resp = await cdnGet(playlistUrl);
