...
export function changeCredentials(
  ...
  const auth = loadAuth();
  // 验证当前密码(恒定时间比较,防时序侧信道)
  const currentHash = hashPassword(currentPassword, auth.salt);
  if (!safeEqualHex(currentHash, auth.passwordHash)) {
