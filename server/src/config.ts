...
export const DEFAULT_SETTINGS: Settings = {
  // 默认 5 秒(用户要求);注意:过快的轮询会对快手/抖音/小红书等
  // 每主播每轮都要请求页面的平台造成限流风险,视平台情况可调高
  pollIntervalSec: 5,
  segmentDuration: '',
  segmentFileSize: '',
  downloader: 'ffmpeg',
  ...
  // 默认上传器:grammY(Telegram 上传);如改用 rclone 可在后处理页面切换
  uploadTool: 'grammy',
  // 网盘类型(支持账号密码登录的网盘 id:pikpak / mega)
...
# rclone 已内置在容器内,配置文件由 Web UI「后处理 → rclone」自动生成/更新
# 环境变量:
...
#   CAPTUREHUB_RCLONE        rclone 可执行文件 (默认 /usr/bin/rclone,镜像内置)
...
RCLONE="\${CAPTUREHUB_RCLONE:-/usr/bin/rclone}"
...
# 配置文件由 Web UI「后处理 → rclone」自动生成/更新;
# 此处重试机制确保等待配置就绪(如刚保存账号密码时)
CONFIG_FOUND=""
RETRY=0
MAX_RETRY=15
...
if [ -z "$CONFIG_FOUND" ]; then
  ...
  echo "  请打开 Web UI「后处理 → rclone」填写网盘账号密码并保存"
  exit 1
fi
...
`,
  ...
  rclonePath: process.env.RCLONE_PATH || '/usr/bin/rclone',
