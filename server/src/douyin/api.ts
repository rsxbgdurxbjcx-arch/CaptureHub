...
async function getRoomInfoByWeb(
  ...
  assert(
    ...
    // 注意:错误消息中不包含 Cookie 等敏感凭证,避免经 API 响应/数据库/日志泄露
    `Unexpected resp, code ${respData.status_code}, msg ${JSON.stringify(respData.data)}, id ${webRoomId}`,
  );
...
/**
 * 解析 main.sdk_params(JSON 字符串),取视频编码标识 VCodec(如 "264"/"265"/"7" 等)。
 * 与上游 streamget 一致: VCodec 缺失/解析失败返回 null。
 */
export function parseSdkVCodec(main: { sdk_params?: string } | undefined): string | null {
  if (!main?.sdk_params) return null;
  try {
    const params = JSON.parse(main.sdk_params) as Record<string, unknown>;
    const v = params?.VCodec;
...
/**
 * 在流 URL 上追加 codec={VCodec} 参数(与上游 streamget 一致)。
 * 抖音 CDN 的流 URL 若不显式携带 codec 参数,返回的编码可能与声明不符
 * (可能被切换为 H.265 / ByteVC1),导致录制后无法正常播放。
 * URL 已含 codec 参数时跳过,避免重复。
 */
export function withCodecParam(url: string | undefined, vCodec: string | null): string | undefined {
  if (!url || !vCodec) return url;
  if (/[?&]codec=/i.test(url)) return url;
  return url + (url.includes("?") ? "&" : "?") + "codec=" + encodeURIComponent(vCodec);
}

/**
 * 将 VCodec 归一化为编码类别(精确匹配,避免 "265" 被 "2" 前缀误判为 h264):
 * 264/avc → h264;265/hevc → h265;bytevc1/266/vvc/vc1 → bytevc1(字节自研,FFmpeg 无法解码);
 * 其他未知值返回小写编码名。返回值仅用于流选择与诊断。
 */
export function codecKind(vCodec: string | null): string | undefined {
  if (!vCodec) return undefined;
  const v = vCodec.trim().toLowerCase();
  if (v === "264" || v === "h264" || v === "avc" || v === "avc1") return "h264";
  if (v === "265" || v === "h265" || v === "hevc") return "h265";
  if (v === "bytevc1" || v === "266" || v === "vvc" || v === "vc1") return "bytevc1";
  return v;
}

/**
 * 删除 URL 中的 only_audio 参数(位置可能在 ? 后或 & 后,部分流为纯音频标记)。
 * 使用 URL 类保证参数顺序无关;URL 解析失败时退回正则。
 */
export function stripOnlyAudioParam(url: string): string {
  try {
    const u = new URL(url);
    if (u.searchParams.has("only_audio")) {
      u.searchParams.delete("only_audio");
      return u.toString();
    }
    return url;
  } catch {
    return url.replace(/[?&]only_audio=[^&]*/gi, "").replace(/\?&/, "?");
  }
}
...
export async function getRoomInfo(
  ...
  // 转换流数据结构
  // 注意:每条流的 main.sdk_params(JSON 字符串)携带 VCodec 编码标识;
  // 与上游 streamget 一致,给 URL 追加 codec={VCodec} 参数,确保 CDN 返回
  // 与声明一致的编码(否则可能被切换为 H.265/ByteVC1,录制后出现
  // "只有声音、画面马赛克"等播放异常)。
  const streamList: StreamInfo[] = Object.entries(streamData)
    .map(([quality, info]) => {
      const stream = info?.main;
      const vCodec = parseSdkVCodec(stream);
      const name = qualityList.find((item) => item.key === quality)?.desc;
      return {
        quality: quality,
        name: name ?? "未知",
        flv: withCodecParam(stream?.flv, vCodec),
        hls: withCodecParam(stream?.hls, vCodec),
        codec: codecKind(vCodec),
      };
    })
  ...
  if (!!aoStream) {
    // 真原画流是在ao流中拿到的(only_audio=1 为音频标记,需删除)
    streamList.push({
      quality: "real_origin",
      name: "真原画",
      flv: stripOnlyAudioParam(aoStream?.flv ?? ""),
      hls: stripOnlyAudioParam(aoStream?.hls ?? ""),
      codec: aoStream?.codec,
    });
  }
  streamList.sort((a, b) => {
