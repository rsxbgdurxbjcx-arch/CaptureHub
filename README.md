...
- 七平台直播流解析逻辑:抖音/小红书/哔哩哔哩/快手/SOOP/Pandalive 移植自 [StreamCap](https://github.com/ihmily/StreamCap) 的 `streamget` 模块;Stripchat 拉流逻辑移植自 [StripchatRecorder](https://github.com/ChanTrail/StripchatRecorder)
...
- **六平台独立 Cookie 管理**,互不干扰;未设置平台专属 Cookie 时回退到全局 Cookie
- **SOOP 登录系统**:支持在设置页面配置 SOOP 账号密码,自动登录获取 Cookie
- 下载引擎:**FFmpeg**,默认录制引擎
- **双上传工具**:支持 **grammY**(上传到 Telegram 群组)和 **rclone**(上传到网盘,镜像内置 rclone v1.75.1,在 Web UI 填写网盘账号密码即自动生成配置),可独立配置、互不干扰
...
│       │   ├── rclone.ts     # rclone 配置生成/校验/连接状态(账号密码自动配置)
...
├── recordings/               # 本地录像(卷)
├── logs/
└── config/rclone/            # rclone.conf(卷,由 Web UI 自动生成/更新)
```
...
- 网盘账号(支持账号密码登录的网盘:PikPak / MEGA,仅使用 rclone 上传时需要;rclone 已内置在镜像中,无需在宿主机安装)

---

## 部署(Docker Compose)

部署方式不变。Docker 镜像已内置 telegram-bot-api 二进制(预编译,无需源码编译)、grammY 依赖与 **rclone v1.75.1**(无需宿主机安装 rclone)。默认使用 Docker 缓存构建,首次约 2 分钟,后续增量构建更快。如需强制全量重建:`FORCE_REBUILD=1 ./deploy.sh`。

网盘(rclone)配置不依赖宿主机:部署完成后在 Web UI「后处理 → rclone」选择网盘类型、填写账号密码并保存,系统自动生成/更新 `config/rclone/rclone.conf`(权限 600)并验证连接。
...
脚本会自动完成:Docker 安装 → 拉取代码 → 准备目录 → 构建镜像 → 启动容器 → 全链路验证(含容器内 rclone v1.75.1 检查)。
...
# → {"ok":true,"message":"CaptureHub ok","version":"v2.4.1"}

# 4b. 系统状态(确认工具可用性)
curl http://127.0.0.1:3780/api/system/status | python3 -m json.tool
# 确认 "tools": { "ffmpeg": true, "telegramBotApi": true, "rclone": true, ... }

# 4c. 容器内 rclone 版本(应为 v1.75.1)
docker compose exec capturehub rclone version | head -n1

# 4d. 网盘配置完成后(Web UI「后处理 → rclone」保存后)可执行完整链路验证:
docker compose exec capturehub sh /app/verify.sh
...
# 验证
curl http://127.0.0.1:3780/api/health
docker exec capturehub rclone version | head -n1
# 网盘配置:登录 Web UI → 后处理 → rclone,填写网盘类型/账号/密码并保存后自动生成配置
```
...
**rclone 已内置在 Docker 镜像中(固定 v1.75.1,支持 linux/amd64 与 linux/arm64)**,无需在宿主机安装,也无需手动执行 `rclone config`。

- 配置文件:`/config/rclone/rclone.conf`(持久化于项目 `config/rclone/` 目录,容器重建不丢失)
- 权限:600(由应用自动维护)
- 上传调用:容器内 `/usr/bin/rclone`,并显式指定 `--config /config/rclone/rclone.conf`

### Web UI 配置(推荐)

1. 打开 Web UI → **后处理** → **rclone** 卡片
2. 点击「网盘类型」选择网盘(仅显示支持账号密码登录的网盘:**PikPak / MEGA**)
3. 填写「网盘账号」与「网盘密码」(填写完成后自动保存并连接,无需手动提交)
4. 系统调用 rclone 官方流程自动生成/更新 `config/rclone/rclone.conf`(含登录认证)并验证连接;卡片顶部实时显示连接状态(已连接 / 未连接)与错误提示,检测在后台静默进行

密码经 rclone 官方 `obscure` 混淆后传入,后端调用 **rclone 官方配置流程**(`rclone config create` 非交互模式)自动完成账号密码登录、获取访问 token 并生成完整配置;**重新提交凭据时会先删除旧 remote 配置段再重新生成,确保旧账号/token 无残留**;账号、密码不会出现在日志、错误信息或 API 返回值中。

### 连接状态查询

```bash
# 容器内查看 remote 列表与连通性
docker compose exec capturehub rclone listremotes --config /config/rclone/rclone.conf
...
4. **设置** → 轮询间隔(默认 5 秒)、切片时长(默认 `03:00:00`,留空不切片)、**切片文件大小(MB)**(留空不切片)、默认录制清晰度
5. **后处理** → 选择上传工具(grammY 或 rclone):
   ...
   - **rclone 配置**:选择网盘类型(PikPak / MEGA)→ 填写网盘账号与密码(填写完成后自动保存并连接)→ 按需调整网盘根目录(默认 `capturehub`)与模式(move/copy)
...
| **rclone** | `RCLONE_PATH` | `/usr/bin/rclone` | 网盘上传工具,镜像内置(v1.75.1,amd64/arm64),无需宿主机安装 |
| **telegram-bot-api** | `TELEGRAM_BOT_API_PATH` | `telegram-bot-api`(PATH 查找) | Local Bot API Server 二进制,镜像内置 |

> 说明:rclone 配置由 Web UI 自动生成/更新,配置文件位于 `/config/rclone/rclone.conf`;其他工具如需自定义路径,设置对应环境变量即可(见 `.env.example`)。
...
**说明**:抖音的 webRid (roomId) 是稳定的,不需要每场更新。系统自动获取 ttwid Cookie 并生成 ABogus / __ac_signature 签名,**无需配置 Cookie 即可解析**。如配置了 Cookie 可获得更稳定的解析效果。抖音部分直播流为 **H.265/ByteVC1 编码**(流接口的 `sdk_params.VCodec` 标识,系统自动为流 URL 追加 `codec=` 参数与上游 streamget 保持一致,并优先选择可解码的 H.264 流);H.265 流上传 Telegram 前会自动转码为 H.264,ByteVC1(字节自研编码,FFmpeg 无法解码)流会自动跳过。
...
**说明**:Stripchat 的 username 是稳定的,不需要每场更新。系统通过 broadcasts API + cam API 检测直播状态(2026-08 起 Stripchat 停用旧的 `models/username/{username}/cam` 端点,脚本请求返回 418;新流程为 `v1/broadcasts/{username}` → `v2/models/{streamName}/cam`,并保留主页 HTML `__PRELOADED_STATE__` 兜底),通过多 CDN 域名竞速解析 master 播放列表获取最高画质流,支持 Mouflon 加密密钥配置。仅 public(公开秀)状态可录制,私密/群组秀等需登录或令牌的直播无法录制。添加主播后显示名称自动设为主播 username。
...
步骤1: GET https://stripchat.com/api/front/v1/broadcasts/{username}
  解析 item.streamName (模型索引)
步骤2: GET https://stripchat.com/api/front/v2/models/{streamName}/cam
  解析 isLive / status / viewersCount / id (modelId) / 缩略图
  仅 status == "public" 可录制
  (旧端点 models/username/{username}/cam 已停用,脚本请求返回 418;
   另有模型主页 HTML window.__PRELOADED_STATE__ 兜底)
步骤3: 多 CDN 顶级域名竞速获取 master 播放列表
...
步骤5: 按画质选择最佳变体
  OD(原画) > UHD(超清) > HD(高清) > SD(标清) > LD(流畅)
步骤6: 变体 URL 追加 Mouflon 参数 psch/pkey → 最终 HLS 地址
  若配置了 Mouflon 密钥,优先使用已匹配 pkey 的 pair
步骤7: 持续拉取变体 m3u8,解析分片列表
  - 解析 #EXT-X-MOUFLON:PSCH 获取当前 pkey
  - 解析 #EXT-X-MOUFLON:URI: 加密分片,用 pdkey 做 SHA-256 XOR 解密
  - 解析 EXT-X-MAP 获取 fMP4 init 段
步骤8: 多 CDN 竞速下载分片(init + media),fMP4 转 TS 写入会话目录
步骤9: 录制结束 ffmpeg concat 合并 TS 分片为 mp4/ts
```
...
系统支持六平台独立 Cookie 管理,互不干扰。在 **设置** 页面可为每个平台单独配置 Cookie。
...
- **登录限流**:同一 IP 15 分钟内连续失败 10 次将被锁定,防止暴力破解
- **媒体鉴权**:`/media/*` 录像预览需携带 Bearer Token 或 `?token=` 参数(前端自动附带)
- **CORS**:默认不开启跨域;如需跨域调用,通过环境变量 `CORS_ORIGIN`(逗号分隔来源)配置
...
- **原理**:调用容器内置 rclone(v1.75.1),读取 `/config/rclone/rclone.conf` 上传到网盘
- **配置**:网盘类型(PikPak / MEGA,账号密码登录)、网盘账号、网盘密码、根目录、模式、后处理脚本
- **连接状态**:卡片实时显示真实结果(已连接 / 未连接 + 错误提示;检测后台静默进行)
- **模式**:move / copy
...
- **Telegram 兼容性保证**:上传前自动 ffprobe 检测编码,非 H.264(如抖音 H.265/ByteVC1)、非 AAC 音频、奇数分辨率、非 yuv420p 等情况自动转码为 H.264+AAC(CRF 20,零开销直传符合要求的文件),彻底修复「只有声音、画面全是马赛克」的播放问题
...
| `CAPTUREHUB_RCLONE` | rclone 可执行文件路径(默认 `/usr/bin/rclone`,镜像内置) |
...
- rclone 配置文件(`/config/rclone/rclone.conf`)权限自动设为 `600`
...
| GET | `/api/postprocess/rclone/status` | rclone 连接状态(支持 `?force=1` 强制检测) |
| POST | `/api/postprocess/rclone/connect` | 保存网盘账号密码,自动生成/更新 rclone 配置并验证连接 |
...
| GET | `/media/*` | 本地录像流媒体预览(需 Bearer 或 `?token=` 鉴权) |
...
## 故障排除:磁盘爆满 / 文件堆积 / Web UI 卡死

### 现象

- 录制文件不断堆积,磁盘使用率持续上涨直至 95%+;
- Web UI 变卡、请求超时,甚至完全无法打开;
- 任务记录中出现大量「失败」/「错误」卡片;
- Stripchat 主播文件夹下出现 `.<文件名>_segments` 隐藏目录(内含大量 `.ts` 分片)。

### 根因(旧版本)

1. **上传失败(尤其大文件在低带宽 VPS 上超时)后文件被标记为永久 `error` 且不自动重试** —— 文件留在磁盘上,任务永远卡死,上传不会再次发生,导致堆积;
2. Stripchat 分片录制进程被**强杀/OOM/崩溃**时,分片临时目录 `.{base}_segments` 不会被清理(每个可能占数 GB),且磁盘同步会跳过隐藏目录、Web UI 不显示,属于"隐形磁盘占用";
3. 前端每秒轮询 `/api/files` 触发后端**全盘递归扫描**(同步阻塞),文件多时服务器被拖垮,Web UI 表现为"崩溃";
4. 磁盘满之后再发生任何转码/合并/上传/删除都会失败,形成恶性循环。

### 紧急恢复(无需升级即可先缓解)

```bash
# 进入部署目录,先看占用(不删除)
sh scripts/cleanup-disk.sh --report

# 清理所有可安全删除的残留物(Stripchat 分片目录、兼容转码临时文件、临时脚本)
sh scripts/cleanup-disk.sh

# 仍不足?查看最大的 30 个录制文件,确认后手动删除
find recordings -type f \( -name '*.mp4' -o -name '*.ts' -o -name '*.flv' \) -size +100M \
  -printf '%s %p\n' | sort -rn | head -n 30

# 恢复服务(可选:重启前先通过 API 清理残留状态并删除不再存在的文件记录)
curl -X POST http://127.0.0.1:3780/api/system/recover
```

> **重要**:`cleanup-disk.sh` 只删除 `.*_segments` 分片目录、`.tg_compat_*.mp4` 临时文件和 `post_*.sh` 脚本,**不会删除任何成品视频**;成品文件请先确认已上传后再手动删除。上传完成但本地未删除的旧文件(`uploaded` 状态、`remotePath` 非空)可放心删除。

### 根治(升级到最新版)

新版本已修复:
- **Telegram 兼容性误判修复(Stripchat 无法上传的直接原因)**:`yuvj420p`(full-range YUV420P,Stripchat 等平台源流常见)不再被判定为不兼容 → **直传,不再重编码**;旧版本每次上传前都要把 27 分钟视频重编码约 30 分钟并额外写一份近似大小的临时文件,磁盘紧张/转码中断时任务永远卡死;
- 确需转码时(H.265/ByteVC1 等):重编码参数 medium→**fast**(提速约 35%),转码产物在失败重试时**复用**(不再重复转码半小时),上传成功后才删除,启动时自动清理残留;
- 上传失败(超时/网络/限流)后**自动延迟重试(最多 3 次,指数退避)**,全部失败才标记 `error`,失败文件保持可手动重试;
- grammY 上传动态超时按 **0.2 MB/s 保守带宽**估算(旧版 0.5 MB/s 对 1.8GB 文件必然超时),上限放宽至 12 小时;
- 启动时与「系统修复」(`POST /api/system/recover`)自动清理残留的 Stripchat 分片目录(启动时另清理无主的 `.tg_compat_*` 临时文件);
- 磁盘剩余低于 1GB 时**拒绝开始新录制**;转码前检查剩余空间,不足时跳过转码直传并明确提示;
- `/api/files` 列表改为 30 秒节流同步(不再每次请求全盘扫描),Web UI 保持流畅;
- 恢复流程不再为"文件已不存在的残留记录"创建永久 `error` 卡片(直接删除记录);
- ffmpeg 强制 `-nostats`:转码不再每秒输出进度行刷爆任务日志(每行一次写库)。
...
# 3. 在 Web UI「文件」页对失败/就绪卡片逐个点击「上传」(旧版失败文件会被新版自动重试接管)
```
